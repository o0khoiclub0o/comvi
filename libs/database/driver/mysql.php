 ...
 * @subpackage	Database
 */
class CDatabaseMySQL extends CDatabase
{
	/**
	...
	function __construct($options)
	{
		...
		if (!$this->test()) {
			throw new Exception('LIB_DATABASE_ERROR_ADAPTER_MYSQL');
		}

		// Connect to the server
		if (!($this->_connection = @mysql_connect($host, $user, $password))) {
			throw new Exception('LIB_DATABASE_ERROR_CONNECT_MYSQL');
		}
		...
		if (!mysql_select_db($database, $this->_connection)) {
			throw new Exception('LIB_DATABASE_ERROR_DATABASE_CONNECT');
		}
	}
	...
	public function query()
	{
		...
		if (!$this->_cursor) {
			...
			if ($this->_debug) {
				CError::raiseError(500, 'CDatabaseMySQL::query: '.$this->_errorNum.' - '.$this->_errorMsg);
			}
			return false;
		}
		return $this->_cursor;
